...
status_code
|
HTTP status code to send to the client. Read more about it in the FastAPI docs for Handling Errors
TYPE:
|
detail
|
Any data to be sent to the client in the Read more about it in the FastAPI docs for Handling Errors
TYPE:
|
headers
|
Any headers to send to the client in the response. Read more about it in the FastAPI docs for Handling Errors
TYPE:
|
...
reason
|
The reason to close the WebSocket connection. It is UTF-8-encoded data. The interpretation of the reason is up to the application, it is not specified by the WebSocket specification. It could contain text that could be human-readable or interpretable by the client code, etc.
TYPE:
|
Source code in fastapi/exceptions.py
def __init__(
