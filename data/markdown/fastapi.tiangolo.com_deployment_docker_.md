...
Tip
In a hurry and already know this stuff? Jump to the Dockerfile
below 👇.
Dockerfile Preview 👀
FROM python:3.14
WORKDIR /code
...
The most common way to do it is to have a file requirements.txt
...
And you would normally install those package dependencies with pip
, for example:
...
- Create an
app
directory and enter it. - Create an empty file
__init__.py
. - Create a
main.py
file with:
...
/code
directory.Copy only the file with the requirements first, not the rest of the code.
...
was going to be run again to install the same packages, but that's not the case when working with containers.Note
The
--no-cache-dir
is only related topip
, it has nothing to do with Docker or containers.The
--upgrade
option tellspip
to upgrade the packages if they are already installed.Because the previous step copying the file could be detected by the Docker cache, this step will also use the Docker cache when available.
...
directory.As this has all the code which is what changes most frequently the Docker cache won't be used for this or any following steps easily.
So, it's important to put this near the end of the
Dockerfile
, to optimize the container image build times. -
Set the command to use
fastapi run
, which uses Uvicorn underneath.CMD
...
/code
directory you set above withWORKDIR /code
.
Tip
Review what each line does by clicking each number bubble in the code. 👆
Warning
Make sure to always use the exec form of the CMD
...
This can be quite noticeable when using docker compose
. See this Docker Compose FAQ section for more technical details: Why do my services take 10 seconds to recreate or stop?.
Directory Structure¶
...
, we first copy the file with the dependencies alone, not the rest of the code. Let me tell you why is that.
COPY ./requirements.txt /code/requirements.txt
Docker and other tools build these container images incrementally, adding one layer on top of the other, starting from the top of the Dockerfile
and adding any files created by each of the instructions of the Dockerfile
.
...
Then, near the end of the Dockerfile
, we copy all the code. As this is what changes most frequently, we put it near the end, because almost always, anything after this step will not be able to use the cache.
COPY ./app /code/app
...
If your FastAPI is a single file, for example, main.py
without an ./app
...
main.py
file to the/code
directory directly (without any./app
directory). -
Use
fastapi run
to serve your application in the single filemain.py
.
When you pass the file to fastapi run
...
So, in this case, you would not want to have a multiple workers in the container, for example with the --workers
command line option. You would want to have just a single Uvicorn process per container (but probably multiple containers).
...
In those cases, you can use the --workers
...
And if you need to have multiple workers, you can simply use the --workers
command line option.
Technical Details
...
But now that Uvicorn (and the fastapi
command) support using --workers
...
Taking care of the order of instructions in the Dockerfile
and the Docker cache you can minimize build times, to maximize your productivity (and avoid boredom). 😎
