...
or Optional
¶
If your code for some reason can't use |
, for example if it's not in a type annotation but in something like response_model=
, instead of using the vertical bar (|
) you can use Union
from typing
.
For example, you could declare that something could be a str
or None
:
...
print(f"Hi {name}!")
typing
also has a shortcut to declare that something could be None
, with Optional
.
...
The good news is, in most cases, you will be able to simply use |
to define unions of types:
def say_hi(name: str | None):
print(f"Hey {name}!")
So, normally you don't have to worry about names like Optional
and Union
. 😎
